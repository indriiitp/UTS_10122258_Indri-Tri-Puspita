        ...
        <div class="boxJudul textbirutua">
            <div class="row">
                <div class="col" style="font-size:20px">
                    <h4><b>Form Ubah Pegawai </b></h4>
                </div>
            </div>
        </div>
    </div>
    <div class="boxIsi textbirutua">
        <form method="POST" action="Controller/cUpdatePegawai.php?aksi=edit">
            <?php
            if (isset($_GET["id"])) {
                $id = $_GET["id"];
                $data = $pegawai->ubahPegawaiById($id);

                foreach ($data as $dPegawai) {
                    ?>
                    <div class="mb-3">
                        <label for="iIDUser" class="form-label">ID User</label>
                        <input type="text" class="form-control" name="id_user" value="<?php echo $dPegawai["id_user"]; ?>" readonly>
                    </div>
                    <div class="mb-3">
                        <label for="iUsername" class="form-label">Username</label>
                        <input type="text" class="form-control" name="username" value="<?php echo $dPegawai["username"]; ?>">
                    </div>
                    <div class="mb-3">
                        <label for="iPassword" class="form-label">Password</label>
                        <input type="password" class="form-control" name="password" value="<?php echo $dPegawai["password"]; ?>">
                    </div>
                    <div class="mb-3">
                        <label for="iNama" class="form-label">Nama</label>
                        <input type="text" class="form-control" name="nama" value="<?php echo $dPegawai["nama"]; ?>">
                    </div>
                    <div class="mb-3">
                        <label for="iNohp" class="form-label">Nomor Handphone</label>
                        <input type="text" class="form-control" name="no_hp" value="<?php echo $dPegawai["no_hp"]; ?>">
                    </div>
                    <div class="mb-3">
                        <label for="iAlamat" class="form-label">Alamat</label>
                        <input type="text" class="form-control" name="alamat" value="<?php echo $dPegawai["alamat"]; ?>">
                    </div>
                    <div class="mb-3">
                        <label for="iJabatan" class="form-label">Jabatan</label>
                        <select id="iJabatan" name="jabatan" class="form-control" value="<?php echo $dPegawai["jabatan"]; ?>">
